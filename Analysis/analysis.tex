...
\subsection{Hyper-V}
Hyper-V is a hypervisor product developed by Microsoft. It %here

On the contrary to VirtualBox, with its lack of server-oriented packages, Hyper-V has two primary versions (Though there a sub-versions within these), one is a server-specific version designed to work on windows server, and the other is a stand-alone version designed to work on most windows %and here


\subsection{VMware}
%Mention the network manager and adapters along with bridged mode for linking to real networks. Mention that is has server-oriented packages.
