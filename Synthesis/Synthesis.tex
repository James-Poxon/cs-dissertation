...
This motherboard supports up to a maximum of 16GB of RAM, and a frequency of 1600Mhz in dual channel which we are taking full advantage of with two 8GB sticks of 1600Mhz RAM. The CAS latency is important here too. CAS means ``Column Address Strobe'', and means the number of times the memory clock cycles between the `read' command and the data actually being read \citep{CASlatency}. Generally the lower the CAS latency, the better for sheer speed. The intricacies of memory profiles and CAS latencies is far beyond the scope of this report, but it is important to mention in the event someone wants to check the results of this study with their own research in the future. As with any benchmarking test, small changes in hardware can big consequences on results, so being as transparent as possible with regards to the hardware specification is within our best interest. CAS latency can make a noticeable difference in real-world speed of RAM, so even if two RAM sticks are the same Frequency, having different CAS latencies could shift results.

The graphics card should little difference to the results, but is worth mentioning nonetheless.
